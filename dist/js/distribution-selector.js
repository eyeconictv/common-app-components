...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('distribution-selector/distribution-list.html',
    '<div ng-controller="distributionListController"><search-filter filter-config="filterConfig" search="search" do-search="doSearch"></search-filter><div class="panel half-top"><div class="scrollable-list" scrolling-list="load()" rv-spinner="" rv-spinner-key="display-list-loader" rv-spinner-start-active="1"><div class="text-center add-top-double" ng-if="!loadingDisplays && displays.list.length === 0"><h4 class="text-muted add-bottom add-top">No Displays Available</h4><button ng-click="addDisplay()" class="btn btn-primary btn-lg">Add Display <i class="fa fa-plus icon-right"></i></button></div><table id="displayListTable" class="table table--hover table--selector multiple--selector animated fadeIn" ng-if="displays.list.length > 0"><thead class="table-header"><tr class="table-header__row"><th id="tableHeaderName" ng-click="sortBy(\'name\')" class="table-header__cell clickable">Name<i ng-if="search.sortBy == \'name\'" class="fa" ng-class="{false: \'fa-long-arrow-up\', true: \'fa-long-arrow-down\'}[search.reverse]"></i></th><th id="tableHeaderAddress" class="table-header__cell hidden-xs">Address</th></tr></thead><tbody class="table-body"><tr class="table-body__row clickable display" ng-click="toggleDisplay(display.id);" ng-class="{\'active\' : isSelected(display.id) }" ng-repeat="display in displays.list"><td id="displayName-{{display.id}}" class="table-body__cell display-name"><span>{{display.name}}</span></td><td id="displayAddress-{{display.id}}" class="table-body__cell display-address hidden-xs"><span class="text-muted">{{display.address}}</span></td></tr></tbody></table></div></div></div>');
}]);
})();
